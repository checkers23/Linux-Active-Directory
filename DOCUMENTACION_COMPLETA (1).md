...
> 📸 **Captura:** `images/03_hostname.png` — Salida de hostnamectl y hostname -f mostrando el nombre correcto.
...
> ⚠️ Eliminar cualquier línea que asocie `127.0.1.1` con el hostname anterior. Solo debe quedar la línea con la IP de la red interna.

```bash
cat /etc/hosts
```

> 📸 **Captura:** `images/04_hosts.png` — Contenido de /etc/hosts con la IP interna del servidor.

### 2.3 Configurar interfaces de red con Netplan

El servidor tiene **dos interfaces de red** configuradas en VirtualBox:

- **enp0s3** → **NAT** → para salir a internet e instalar paquetes. IP dinámica asignada por VirtualBox.
- **enp0s8** → **Red interna** (`intnet`) → para el tráfico del dominio AD. IP estática `192.168.10.1/24`.
...
Explicación:
- `enp0s3` en DHCP: el NAT de VirtualBox le asigna IP automáticamente para tener internet.
- `enp0s8` estática `192.168.10.1`: IP del DC en la red interna. El DNS apunta a `127.0.0.1` porque el propio servidor gestionará el DNS del dominio tras el provisioning.
...
> 📸 **Captura:** `images/05_netplan_yaml.png` — Contenido del archivo netplan con las dos interfaces.
> 📸 **Captura:** `images/06_ip_a.png` — Salida de ip a mostrando enp0s3 con IP NAT y enp0s8 con 192.168.10.1.
> 📸 **Captura:** `images/07_ping_internet.png` — Ping a 8.8.8.8 con respuesta correcta.

### 2.4 Deshabilitar IPv6

IPv6 puede causar conflictos con Samba y Kerberos cuando el sistema intenta resolver nombres por IPv6 antes que por IPv4. En un entorno de laboratorio es recomendable desactivarlo.
...
> 📸 **Captura:** `images/08_ipv6_desactivado.png` — Salida de sysctl -p y de ip a sin direcciones inet6.
...
> 📸 **Captura:** `images/09_resolved_disabled.png` — Salida de ss -tulnp sin nada en el puerto 53.
...
> 📸 **Captura:** `images/10_ntp_sync.png` — Salida de ntpq -p y timedatectl mostrando sincronización activa.
...
> 📸 **Captura:** `images/11_domain_provision.png` — Salida completa del comando samba-tool domain provision.
...
> 📸 **Captura:** `images/12_samba_status.png` — Salida de systemctl status samba-ad-dc mostrando active (running).
...
> 📸 **Captura:** `images/13_puertos_ad.png` — Salida de ss mostrando todos los puertos AD activos.
...
> 📸 **Captura:** `images/14_dns_check.png` — Salida de los comandos host mostrando resolución correcta de registros SRV y A.
...
> 📸 **Captura:** `images/15_kinit_klist.png` — Salida de klist mostrando el ticket Kerberos del administrador.
...
> 📸 **Captura:** `images/16_smbclient.png` — Listado de shares del servidor mediante smbclient.
...
> 📸 **Captura:** `images/17_cliente_ipv6_desactivado.png` — Salida de ip a en el cliente sin direcciones inet6.
...
> 📸 **Captura:** `images/18_realm_discover.png` — Salida de realm discover mostrando información del dominio DEATHSTAR.LOCAL.
...
> 📸 **Captura:** `images/19_realm_join.png` — Salida de realm join (sin errores indica éxito).
...
> 📸 **Captura:** `images/20_realm_list_id.png` — Salida de realm list y del comando id mostrando el usuario de dominio.
...
> 📸 **Captura:** `images/21_usuarios_grupos.png` — Salida de samba-tool user list y group listmembers jedis.
...
> 📸 **Captura:** `images/22_smb_blueprints.png` — Sección blueprints del archivo smb.conf.
...
> 📸 **Captura:** `images/23_family_web.png` — Navegador mostrando el contenido de /sw/family en http://192.168.10.1/family.
...
> 📸 **Captura:** `images/24_force_web.png` — Navegador mostrando el contenido de /sw/force en http://192.168.10.1/force.
...
> 📸 **Captura:** `images/25_shares_verificacion.png` — Salida de smbclient mostrando los shares accesibles para cada usuario.
...
> 📸 **Captura:** `images/26_bespin_provision.png` — Salida del provisioning del dominio cloud01.city en bespin01.
...
> 📸 **Captura:** `images/27_trap_lando.png` — Acceso exitoso de lando al share trap con smbclient.
> 📸 **Captura:** `images/28_trap_boba.png` — Acceso denegado de boba al share trap.
...
> 📸 **Captura:** `images/29_ssh_conexion.png` — Terminal mostrando la sesión SSH establecida con el servidor.
...
> 📸 **Captura:** `images/30_sl_signals.png` — Terminal mostrando sl lanzado, pausado y reanudado (con cualquiera de los dos métodos).
...
> 📸 **Captura:** `images/31_r2d2_script.png` — Contenido del script r2d2.sh mostrado con cat.
...
> 📸 **Captura:** `images/32_crontab.png` — Salida de crontab -l mostrando la tarea programada a las 10:30.
...
> 📸 **Captura:** `images/33_luke_sky.png` — Salida de ls mostrando el fichero luke.sky creado y el log de r2d2.
...
> 📸 **Captura:** `images/34_trust_create.png` — Salida del comando samba-tool domain trust create mostrando el trust creado.
...
> 📸 **Captura:** `images/35_trust_verify.png` — Salida de samba-tool domain trust list y validate mostrando el trust activo.
...
> 📸 **Captura:** `images/36_trust_kinit.png` — Ticket Kerberos de un usuario de LAB01.LAN obtenido desde DEATHSTAR.LOCAL.

---

## Resumen de capturas necesarias

> Las capturas se van haciendo durante la realización del laboratorio y se suben a la carpeta `images/` del repositorio.

| Nº  | Archivo                              | Apartado       | Qué debe mostrar                                              |
|-----|--------------------------------------|----------------|---------------------------------------------------------------|
| 01  | `01_lsblk_discos.png`               | Discos         | `lsblk` con /dev/sda en / y /dev/sdb en /home                |
| 02  | `02_df_home.png`                    | Discos         | `df -h` confirmando montaje de /home                          |
| 03  | `03_hostname.png`                   | Red            | `hostnamectl` y `hostname -f` mostrando el nombre correcto    |
| 04  | `04_hosts.png`                      | Red            | Contenido de /etc/hosts                                       |
| 05  | `05_netplan_yaml.png`               | Red            | Contenido del archivo netplan con las dos interfaces          |
| 06  | `06_ip_a.png`                       | Red            | `ip a` con enp0s3 (NAT) y enp0s8 (192.168.10.1)              |
| 07  | `07_ping_internet.png`              | Red            | Ping a 8.8.8.8 con respuesta correcta                         |
| 08  | `08_ipv6_desactivado.png`           | Red            | `sysctl -p` e `ip a` sin direcciones inet6                    |
| 09  | `09_resolved_disabled.png`          | Red            | `ss -tulnp` sin nada en el puerto 53                          |
| 10  | `10_ntp_sync.png`                   | NTP            | `ntpq -p` y `timedatectl` con sync activa                     |
| 11  | `11_domain_provision.png`           | Samba          | Salida completa de `samba-tool domain provision`              |
| 12  | `12_samba_status.png`               | Samba          | `systemctl status samba-ad-dc` activo                         |
| 13  | `13_puertos_ad.png`                 | Samba          | `ss -tulnp` con puertos 53,88,389,445,636,3268                |
| 14  | `14_dns_check.png`                  | Kerberos/DNS   | Resolución de registros SRV y A                               |
| 15  | `15_kinit_klist.png`                | Kerberos/DNS   | `klist` con ticket de administrator + `kdestroy`              |
| 16  | `16_smbclient.png`                  | Kerberos/DNS   | Listado de shares con `smbclient`                             |
| 17  | `17_cliente_ipv6_desactivado.png`   | Cliente Linux  | `ip a` del cliente sin inet6                                  |
| 18  | `18_realm_discover.png`             | Cliente Linux  | `realm discover DEATHSTAR.LOCAL`                              |
| 19  | `19_realm_join.png`                 | Cliente Linux  | `realm join` completado sin errores                           |
| 20  | `20_realm_list_id.png`              | Cliente Linux  | `realm list` e `id leia@DEATHSTAR.LOCAL`                      |
| 21  | `21_usuarios_grupos.png`            | Usuarios       | `samba-tool user list` y `group listmembers jedis`            |
| 22  | `22_smb_blueprints.png`             | Shares         | Sección blueprints de smb.conf                                |
| 23  | `23_family_web.png`                 | Shares         | Navegador con /sw/family en http://192.168.10.1/family        |
| 24  | `24_force_web.png`                  | Shares         | Navegador con /sw/force en http://192.168.10.1/force          |
| 25  | `25_shares_verificacion.png`        | Shares         | smbclient verificando acceso por usuario                      |
| 26  | `26_bespin_provision.png`           | Bespin         | Provisioning de cloud01.city en bespin01                      |
| 27  | `27_trap_lando.png`                 | Bespin         | Acceso de lando al share trap                                 |
| 28  | `28_trap_boba.png`                  | Bespin         | Acceso denegado de boba al share trap                         |
| 29  | `29_ssh_conexion.png`               | SSH            | Sesión SSH establecida desde la máquina real                  |
| 30  | `30_sl_signals.png`                 | SSH            | sl pausado y reanudado con Ctrl+Z/bg/fg o kill -19/-18        |
| 31  | `31_r2d2_script.png`                | Cron           | Contenido del script r2d2.sh con `cat`                        |
| 32  | `32_crontab.png`                    | Cron           | `crontab -l` con tarea a las 10:30                            |
| 33  | `33_luke_sky.png`                   | Cron           | Fichero luke.sky creado y log de r2d2                         |
| 34  | `34_trust_create.png`               | Trust          | Salida de `samba-tool domain trust create`                    |
| 35  | `35_trust_verify.png`               | Trust          | `samba-tool domain trust list` y validate con WERR_OK         |
| 36  | `36_trust_kinit.png`                | Trust          | Ticket Kerberos cruzado entre dominios                        |
