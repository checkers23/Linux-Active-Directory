...
2. [Network and Hostname — srvWXX](#2-network-and-hostname--srvwxx)
...
8. [Linux Client — ucXX](#8-linux-client--ucxx)
9. [Secondary Server — BespinXX (cloudXX.city)](#9-secondary-server--bespinxx-cloudxxcity)
...
## 2. Network and Hostname — srvWXX

### 2.1 Set Hostname

```bash
sudo hostnamectl set-hostname srvWXX.SWXX.local
hostname -f
# Expected: srvWXX.SWXX.local
```
...
```
127.0.0.1       localhost
192.168.1.1     srvWXX.SWXX.local srvWXX
```
...
> When prompted for the Kerberos realm, enter `SWXX.LOCAL` (uppercase).
...
```bash
host -t SRV _kerberos._tcp.SWXX.local 127.0.0.1
host -t SRV _ldap._tcp.SWXX.local 127.0.0.1
host -t A srvWXX.SWXX.local 127.0.0.1
```
...
```bash
kinit administrator@SWXX.LOCAL
...
[force]
    ...
    read only = yes
    guest ok = yes
    valid users = SWXX\anakin, SWXX\yoda, @SWXX\jedis
    invalid users = SWXX\leia
```
...
> All steps in this section are performed on the **client machine** `ucXX`.
...
> ⚠️ The internal network name `intnet` must be identical to the one used on srvWXX.
...
```bash
realm discover SWXX.LOCAL
```

> 📸 `images/23_realm_discover.png` — realm discover output

```bash
sudo realm join --user=Administrator SWXX.LOCAL
# Password: P@ssw0rd2024!
```
...
## 9. Secondary Server — BespinXX (cloudXX.city)

> All steps in this section are performed on the second server `BespinXX`. This simulates an AWS cloud instance with its own independent domain.

### 9.1 Hostname and Hosts

```bash
sudo hostnamectl set-hostname BespinXX.cloudXX.city
sudo nano /etc/hosts
```

```
127.0.0.1       localhost
192.168.1.2     BespinXX.cloudXX.city BespinXX
```
...
```bash
sudo netplan apply
ping -c 4 192.168.1.1   # verify connection to srvWXX
```
...
> 📸 `images/26_bespin_provision.png` — samba-tool domain provision output for cloudXX.city
...
Add to `/etc/samba/smb.conf` on BespinXX:
...
[trap]
    ...
    browseable = no
    read only = no
    valid users = CLOUDXX\lando
    invalid users = CLOUDXX\boba
    create mask = 0660
    directory mask = 0770
```
...
### 10.1 Verify SSH is Running on srvWXX
...
A bidirectional trust is established between `SWXX.LOCAL` (srvWXX) and `LABXX.LAN` (the lab practices server).

### 12.1 Set Up labXX

The `labXX` VM follows the same setup as `srvWXX` with different values:
...
```bash
sudo hostnamectl set-hostname labXX.labXX.lan
```

`/etc/hosts`:
```
127.0.0.1     localhost
192.168.1.4   labXX.labXX.lan labXX
```

Netplan `enp0s8`: IP `192.168.1.4/24`, DNS `127.0.0.1`, search `labXX.lan`
...
host -t SRV _kerberos._tcp.labXX.lan 192.168.1.4
```
...
```bash
ping -c 4 192.168.1.1
host -t SRV _kerberos._tcp.SWXX.local 192.168.1.1
```

### 12.3 Configure DNS Forwarding

**On srvWXX** — add to `[global]` in `/etc/samba/smb.conf`:

```ini
dns forwarder = 192.168.1.4
```

**On labXX** — add to `[global]` in `/etc/samba/smb.conf`:
...
```bash
# From srvWXX
host -t SRV _ldap._tcp.labXX.lan

# From labXX
host -t SRV _ldap._tcp.SWXX.local
```
...
```bash
sudo samba-tool domain trust create LABXX.LAN \
  --type=forest \
  --direction=both \
...
sudo samba-tool domain trust validate LABXX.LAN -U administrator%P@ssw0rd2024!
```
...
```bash
kinit administrator@LABXX.LAN
klist
```

> 📸 `images/36_trust_kinit.png` — Kerberos ticket for LABXX.LAN user obtained from SWXX.LOCAL
...
| 22 | `22_client_ipv6_disabled.png` | Client | ip a on ucXX with no inet6 |
| 23 | `23_realm_discover.png` | Client | realm discover SWXX.LOCAL |
| 24 | `24_realm_join.png` | Client | realm join with no errors |
| 25 | `25_realm_list_id.png` | Client | realm list and id leia@SWXX.LOCAL |
| 26 | `26_bespin_provision.png` | BespinXX | Provisioning cloudXX.city |
| 27 | `27_trap_lando.png` | BespinXX | lando accessing trap share |
| 28 | `28_trap_boba.png` | BespinXX | boba denied access to trap |
